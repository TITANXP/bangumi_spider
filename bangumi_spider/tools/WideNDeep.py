...
def display_metric(train, valid, title):
...
if __name__ == '__main__':
    ...
    history = model.fit(train_dataset,
              ...
              class_weight={0: 0.9, 1: 0.1}, # 正负样本不均衡，因此设置不同的权重
              validation_data=test_dataset
              )
    print(history.history)
    display_metric(history.history['loss'], history.history['val_loss'], 'loss')
    display_metric(history.history['accuracy'], history.history['val_accuracy'], 'accuracy')
    display_metric(history.history['auc'], history.history['val_auc'], 'ROC_AUC')
    display_metric(history.history['auc_1'], history.history['val_auc_1'], 'PR_AUC')
    ...
    tf.keras.models.save_model(
        model,
        "D:/python_project/bangumi_spider/bangumi_spider/resources/model/widendeep6/001", # 001是版本号
        overwrite=True,
    ...
    # windendeep3: 样本数据正序， batchsize: 30 → 600 , 爬取抛弃动画作为负样本
    # windendeep4: 设置正负样本损失函数权重
