...
def get_rating_df():
    ...
    collection = db['user1']
    ratings = collection.find({'collects.0': {'$exists': True}},{'_id': 0, 'user_id': 1, 'collects': 1, 'tags': 1})
...
def get_dropped_df():
    ...
    :return:
    """
    logger.info('加载用户抛弃数据...')
    client = pymongo.MongoClient('localhost', 27017)
    db = client['bangumi_test']
    collection = db['user_dropped']
    ratings = [i for i in collection.find({'droppeds.0': {'$exists': True}},{'_id': 0, 'user_id': 1, 'droppeds': 1})]
    for rating in ratings:
        user_collect = db['user'].find_one({'user_id': rating['user_id']}, {'_id': 0, 'tags': 1, 'collects': 1})
        rating['tags'] = user_collect['tags'] if user_collect else []
        rating['collects'] = user_collect['collects'] if user_collect else []
    rating_df =  pd.DataFrame(list(ratings))
    rating_df.rename(columns={'tags': 'user_tags', 'user_id': 'userId'}, inplace=True)
    ...
    # 按收藏记录进行展开
    rating_df = rating_df.explode('droppeds')
    rating_df = rating_df[pd.notna(rating_df['droppeds'])]
    # 将收藏记录这一列拆成3列
    rating_df['animeId'] = rating_df['droppeds'].apply(lambda x: x[0])
    rating_df['score'] = rating_df['droppeds'].apply(lambda x: x[1])
    rating_df['timestamp'] = rating_df['droppeds'].apply(lambda x: x[2])
    rating_df = rating_df.drop('droppeds', axis=1)
    logger.info('添加lable...')
    rating_df['label'] = 0
    # 4.添加动画特征
...
def get_anime_df():
    ...
    animes = collection.find({},{'_id': 0, 'id': 1, 'api.rating.total': 1, 'api.rating.score': 1, 'tags': 1})
...
def add_label(ratings):
    ...
    # 未评分或评分大于5的label为1
    logger.info('添加lable...')
    ratings['label'] = ratings['score'].apply(lambda score: 1 if score > 5 or score == 0 else 0)
...
def add_user_feature(rating_df):
    ...
    logger.info('添加用户特征...')
    # 用户好评动画id
    rating_df['userRatedAnime1'] = rating_df['like_seq'].apply(lambda item: item[0][0] if len(item) > 0 else None)
    rating_df['userRatedAnime2'] = rating_df['like_seq'].apply(lambda item: item[1][0] if len(item) > 1 else None)
    rating_df['userRatedAnime3'] = rating_df['like_seq'].apply(lambda item: item[2][0] if len(item) > 2 else None)
    rating_df['userRatedAnime4'] = rating_df['like_seq'].apply(lambda item: item[3][0] if len(item) > 3 else None)
    rating_df['userRatedAnime5'] = rating_df['like_seq'].apply(lambda item: item[4][0] if len(item) > 4 else None)
    # 用户平均评分
...
def split_and_save(rating_df):
    """
    按时间切分训练集和测试集，并保存到csv
    :param rating_df:
    :return:
    """
    logger.info('保存到csv...')
    rating_df['timestamp'] = pd.to_datetime(rating_df['timestamp'])
    rating_df = rating_df.sort_values('timestamp', ascending=True)
    split = int(len(rating_df) * 0.97)
    print('训练集数据量', split)
    print('测试集数据量', len(rating_df)-split)
    rating_df[:split].to_csv("trainSample1.csv", index=False)
    rating_df[split:].to_csv("testSample1.csv", index=False)
...
def save_anime_feature_to_redis(rating_df):
    """
    提取并保存动画特征到redis
    :param rating_df:
    :return:
    """
    logger.info('提取并保存动画特征到redis...')
    r = redis.Redis(host="localhost", port=6379, db=0)
    # pipe = r.pipeline()
    for row in rating_df.itertuples():
        anime = {
            'animeId' : getattr(row, 'animeId'),
            'votes': getattr(row, 'votes'),
            'score': getattr(row, 'score'),
            'animeTag1': getattr(row, 'animeTag1'),
            'animeTag2': getattr(row, 'animeTag2'),
            'animeTag3': getattr(row, 'animeTag3'),
        }
        r.set('animeF:'+str(anime['animeId']), str(anime))
    # pipe.execute()
...
if __name__ == '__main__':
    ...
    # 5. 添加用户特征
    rating_df = add_user_feature(rating_df)
    dropped_df = get_dropped_df()
    rating_df = pd.concat([rating_df, dropped_df])
    # 6. 保存
    split_and_save(rating_df)
    print(rating_df)
    print(rating_df.dtypes)
    rating_df['userRatedAnime1'].fillna(0, inplace=True)
    rating_df['userRatedAnime2'].fillna(0, inplace=True)
    rating_df['userRatedAnime3'].fillna(0, inplace=True)
    rating_df['userRatedAnime4'].fillna(0, inplace=True)
    rating_df['userRatedAnime5'].fillna(0, inplace=True)
    # # 7. 提取动画和用户特征，保存到redis
    # save_user_feature_to_redis(rating_df)
    # save_anime_feature_to_redis(rating_df)
